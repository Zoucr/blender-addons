...
Validation: Blender 4.5.3 package build, manifest/archive validation, repository
catalog generation and the headless smoke test in
`tests/blender_camera_path_rig.py`. This checks enabling the extension under
`bl_ext`, preferences registration, rig creation, animation endpoints, aim-path
creation/removal, scene-scoped deletion and disabling/re-enabling.

Interactive panel layout, GPU viewport overlays, baking and other Blender
versions have not been tested in this import. The supplied manifest declares
Blender 4.2+; that compatibility declaration is preserved, not independently
verified across every version.
