...
def preview_burn_in_handler():
    ...
    # Get font properties
    font_path = playblaster.burn_in.font_family
    if not font_path or not os.path.exists(font_path):
        font_path = BFONT_PATH
    else:
        font_path = Path(font_path)
    font_id = blf.load(font_path.as_posix())
    font_size = int(playblaster.burn_in.font_size * factor)
    font_color = playblaster.burn_in.color
    font_margin = int(playblaster.burn_in.margin * factor)
...
def register_or_unregister_preview_handler(
    ...
    we can not access playblaster.burn_in.enable directly, so we provide
    ...
    if mode == "register":
        ...
        register = playblaster.burn_in.enable and playblaster.burn_in.preview
